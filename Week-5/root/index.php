...
<?php
	...
	else// not gallery (show one image)
	{
		...
		if (($show_slideshow)&&(count($results)>1))
		{
			...
			// echo script for changing page content
			if ($objPostInfo->slideshow_enable==true)
			{
				echo "<script language=\"javascript\">window.onload=start_timeout;";
				echo "function start_timeout(){setTimeout(go_to_next_page,$slideshow_speed);}";
				echo "function go_to_next_page(){".CookiesLikeMakeLink($slideshow_link)."}</script>";
			}
		}
